...
function ohSnap(text, options, iSecond) {
    ...
    objectDom = document.getElementById(options['container-id']);
    if (!objectDom) {
        // 基于准备好的dom，初始化echarts实例
        console.log('ohsnap:dom中没有找到容器');
    }
    ...
    if (options.color) {
        color_markup = 'snap-alert-' + options.color;
    }

    // Generate the HTML
    var html = $('<div class="snap-alert ' + color_markup + '">' + icon_markup + text + '</div>').fadeIn(options['fade-duration']);
