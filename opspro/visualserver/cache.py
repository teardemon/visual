...
def CacheQuery():
    ...
    if not jIDCTraff:
        ...
        ExecManagerFunc('alert', 'Alert', 'IDC线路流量不允许为空', IM_GROUP_MAINTAIN)

    jIPTop = query.GetIPTop()
    if not jIPTop:
        str_msg = '机房 所有 Top10流量为空.\n可能是数据的来源异常。\n请手动核对接口数据：{0}'.format(URL_IDC_TOP)
        ExecManagerFunc('log', 'Log', str_msg, 'status/error')
        ExecManagerFunc('alert', 'Alert', str_msg, IM_GROUP_MAINTAIN)
    ...
    if not dDataEchart:
        ExecManagerFunc('alert', 'Alert', 'opspro没能生成echart数据！idc流量图将无法刷新！', IM_GROUP_DEVELOPER)
